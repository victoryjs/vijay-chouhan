...
- **`assets/css/style.css`**: all component/layout styles in one file, organized into clearly marked sections (Reset, Base, Accessibility, Navbar, Buttons, Hero, stat cards, Teaser, Timeline, Certifications, Skills, Projects, Contact, Footer, page-hero, entrance animation, Responsive). Keep new styles inside the matching section rather than appending ad hoc rules at the end.
...
- **Analytics**: every page loads GoatCounter (`data-goatcounter="https://vijaychouhan.goatcounter.com/count"`) via one async script right before `main.js` — cookieless, free, and non-blocking. Keep this line in sync across all 7 HTML files if the site code ever changes.

## Content conventions

- Keep filenames stable when swapping content: profile photo stays `assets/images/profile/vijay-chouhan.jpeg`, resume stays `assets/resume/vijay_chouhan_resume.pdf` (download buttons/links reference these exact paths).
- **Project case studies** (`projects.html`): each project is a full case study, not a short blurb — `.case-study__header` (title + "Private Client Project" badge, or swap the badge for a `.project-card__links` "Visit live site ↗" link if a project ever goes public), then `.case-study__section` blocks for **Challenge**, **My Role**, and **Key Achievements** (the achievements list reuses the `.case-study__achievements` bullet style), then the existing `.tag-row`. Each project `<article>` has an `id` (`#habitake`, `#sootra`, `#myticketpartner`, `#cred-iq`) so the homepage's featured-work cards and any external link can deep-link to a specific case study. Content must trace to the resume PDF or something Vijay explicitly provided — never invent metrics.
- **Homepage featured work** (`index.html`): a compact 3-card teaser (`.project-grid--compact`) reusing `.project-card`/`.project-card__links`, each linking to a `projects.html#anchor`. Keep this in sync with whichever 3 projects are most worth leading with if `projects.html` changes.
- Project cards use styled gradient banners (`.project-card__media--1` through `--4`, one per project) instead of real screenshots; swap in an `<img>` from `assets/images/projects/` when real screenshots become available.
